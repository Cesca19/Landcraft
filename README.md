...
- **Command pattern with undo/redo** : every editing action (elevation, flattening, smoothing,
  painting and procedural generation) is implemented as a reversible command (`src/commands/`)
...
### Terrain editing
**Multiple terrain editing tools**:
  * **Elevation Tool** : raise or dig terrain in real time with continuous brush-based editing
  * **Set Height Tool** : flatten terrain to a target height
  * **Smooth Tool** : smooth height transitions and soften sharp terrain variations
* **Two selection modes**: edit either individual *tile corners* or whole *tiles*
* **Weighted brushes**: 8 brush shapes with adjustable size and intensity
* Continuous stroke interpolation to ensure seamless terrain modification
